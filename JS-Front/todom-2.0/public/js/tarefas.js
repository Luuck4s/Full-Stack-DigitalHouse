...
const onCheckClick = (evt) => {
  ...
  localStorage.setItem("@todom:tasks", JSON.stringify(tarefas));
};
...
const onFormSubmit = async (evt) => {
  ...
  let responseTask = await fetch(`/api/tarefa`, {
    method: "POST",
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
  ...
  responseTask = await responseTask.json();

  tarefas = responseTask.tasks;
